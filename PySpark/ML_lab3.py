...
onlyLabels = parsedDataInit.map(lambda point: int(point.label))
...
# TODO: Replace <FILL IN> with appropriate code
averageTrainYear = (parsedTrainData.map(lambda p:p.label)
                    .mean())
...
def calcRMSE(labelsAndPreds):
    ...
    """
    return labelsAndPreds.map(lambda x:squaredError(x[0],x[1])).mean()**0.5
...
# TODO: Replace <FILL IN> with appropriate code
firstModel = LinearRegressionWithSGD.train(parsedTrainData, iterations = numIters,step = alpha,miniBatchFraction = miniBatchFrac, regParam = reg,
...
def twoWayInteractions(lp):
    ...
    """
    return LabeledPoint(lp.label,np.hstack((lp.features,[i[0]*j[0] for i in itertools.product(lp.features) for j in itertools.product(lp.features)])) )
