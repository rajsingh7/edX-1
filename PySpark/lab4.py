...
movieIDsWithRatingsRDD = (ratingsRDD.map(lambda (uid,mid,rt): (mid,rt))
                                    .groupByKey())
...
movieNameWithAvgRatingsRDD = (moviesRDD.join(movieIDsWithAvgRatingsRDD).map(lambda (k,v): v).map(lambda (k,v):(v[1],k,v[0])))
...
movieLimitedAndSortedByRatingRDD = (movieNameWithAvgRatingsRDD
                                    .filter(lambda (avgRatings, mTitle, numberOfRatings): numberOfRatings > 500)
...
def computeError(predictedRDD, actualRDD):
    ...
    totalError = squaredErrorsRDD.map(lambda (k,v):v).reduce(lambda a,b:a+b)
...
validationForPredictRDD = validationRDD.map(lambda (UserID, MoiveID, Rating):(UserID, MoiveID))
...
myModel = ALS.train(trainingRDD, rank = bestRank, seed=seed, iterations=iterations,
                      lambda_=regularizationParameter)
testForPredictingRDD = testRDD.map(lambda (UserID, MoiveID, Rating):(UserID, MoiveID))
...
myUnratedMoviesRDD = (moviesRDD.filter(lambda (MovieID, MovieTitle): MovieID not in myRatedMovies).map(lambda t: (0,t[0])))
...
movieCountsRDD = movieIDsWithAvgRatingsRDD.map(lambda (movieID,t):(movieID,t[0]))
...
ratingsWithNamesRDD = (predictedWithCountsRDD.join(moviesRDD).map(lambda (k,v):v).map(lambda (k,v):(k[0],v,k[1])))
