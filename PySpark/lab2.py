...
sortedByDay = groupedByDay.map(lambda (k,v): (k, len(v)/len(set(v))))

avgDailyReqPerHost = (sortedByDay.sortByKey()
                      .cache())
...
badEndpointsSum = badEndpointsCountPairTuple.reduceByKey(lambda a,b: a+b).sortBy(lambda x: x[1], ascending = False)
